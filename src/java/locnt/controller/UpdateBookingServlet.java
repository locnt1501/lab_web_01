...
public class UpdateBookingServlet extends HttpServlet {
    ...
    private final String MANAGE_PROCESS_PAGE = "manageProcess";
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            boolean result = dao.updateBookingByStatus(bookingid, status);
            if (result) {
                url = "DispatcherController"
                        + "?txtValue=" + value
                        + "&ddList=" + statusString
                        + "&btAction=SearchBooking";
            }

        } catch (NamingException ex) {
            log("UpdateBookingServlet_SQL " + ex.getMessage());
        } catch (SQLException ex) {
            log("UpdateBookingServlet_Naming " + ex.getMessage());
        } finally {
